...
public class EffAddLevel extends Effect {
	...
	public String toString(@Nullable Event e, boolean debug) {
		return "SkillAPI add %amount% level[s] to %player% from exp source %source%";
	}
	...
	protected void execute(Event e) {
		...
		try {
			...
			SkillAPI.getPlayerData(p).giveLevels(amount, source);
			
		} catch (NullPointerException ex) {
			
			Skript.warning("[SkRPG] Error: NPE at adding levels to class. Did the player have a class in the first place?");
			return;
		}
	}
}
